...
for i in range(len(n_thread_array)):
    ...
    for thread_index in range(num_of_thread):
        file_name_accuracy="{0}/accacy_process_{1}".format(specific_folder_name,thread_index)
        _accuracy=read_list(file_name_accuracy)
        list_of_accuracy.append(_accuracy)

        file_name_loss="{0}/loss_process_{1}".format(specific_folder_name,thread_index)
...
for index_folder_name in range(len(folder_name)):
    ...
    file_name='loss_threads[{0}].png'.format(_today)
...
for index_folder_name in range(len(folder_name)):
    ...
    file_name='accuracy_threads[{0}].png'.format(_today)
    plt.savefig(file_name)
